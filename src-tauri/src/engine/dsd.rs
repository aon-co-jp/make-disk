...
pub fn dsd_sample_rate(multiplier: u32) -> Result<u32, String> {
    match multiplier {
        ...
        other => Err(format!(
            "未対応のDSDレートです: DSD{other}(64/128/256/512/1024のみ) / unsupported DSD rate"
        )),
    }
}

/// DSD出力のおおよそのファイルサイズ(バイト、ステレオ2ch想定)。
pub fn estimate_dsd_size_bytes(
    multiplier: u32,
    channels: u32,
    duration_secs: f64,
) -> Result<u64, String> {
...
impl DeltaSigma {
    fn new(b: &[f64], a: &[f64]) -> Self {
        ...
        Self {
            b: bb,
            a: aa,
            e_hist: [0.0; NTF_ORDER],
            yu_hist: [0.0; NTF_ORDER],
        }
    }
...
fn patch_dsf_sizes<F: Write + Seek>(
    f: &mut F,
...
pub fn convert_to_dsf(
    ...
    let resample = format!(
        ...
        crate::engine::convert::hq_resample_filter()
    );
    cmd.args([
        "-vn",
        "-ac",
    ...
    write_dsf_header(&mut out, sample_rate, channels)
        .map_err(|e| format!("DSFヘッダの書き込みに失敗しました: {e}"))?;

    // チャンネルごとのブロックバッファ(ビットをLSBファーストで詰める)
    let mut blocks: Vec<Vec<u8>> = (0..channels)
        .map(|_| Vec::with_capacity(DSF_BLOCK_BYTES))
        .collect();
    ...
    loop {
        ...
        while carry + n < buf.len() {
            let r = stdout
                .read(&mut buf[carry + n..])
                .map_err(|e| format!("ffmpegの出力の読み取りに失敗しました: {e}"))?;
            if r == 0 {
                break;
            }
            n += r;
        ...
        let bits: Vec<Vec<bool>> = std::thread::scope(|s| {
            ...
            handles
                .into_iter()
                .map(|h| h.join().expect("modulator thread panicked"))
                .collect()
        });
    ...
    let status = child
        .wait()
        .map_err(|e| format!("ffmpegの終了待ちに失敗しました: {e}"))?;
    if !status.success() {
    ...
    patch_dsf_sizes(&mut file, total_samples, data_bytes)
        .map_err(|e| format!("DSFヘッダの更新に失敗しました: {e}"))?;
    Ok(())
}
...
pub fn dsf_to_dop_wav(dsf_path: &str, wav_path: &str, container_bits: u8) -> Result<(), String> {
    ...
    let mut f = std::io::BufReader::new(
        std::fs::File::open(dsf_path).map_err(|e| format!("DSFを開けません: {e}"))?,
    );
    let mut head = [0u8; 92];
    f.read_exact(&mut head)
        .map_err(|e| format!("DSFヘッダを読めません: {e}"))?;
    if &head[0..4] != b"DSD " {
    ...
    let mut out = std::io::BufWriter::with_capacity(
        1 << 20,
        std::fs::File::create(wav_path).map_err(|e| format!("出力を作成できません: {e}"))?,
    );
    let header = open_mqa::wav::encode_wav(&vec![Vec::new(); channels], pcm_rate, container_bits)
        ...
        for i in 0..per_ch[0].len() {
            for ch in &per_ch {
                ...
                if container_bits == 32 {
                    out.write_all(&[0, fr[2], fr[1], fr[0]])
                        .map_err(|e| e.to_string())?;
                    data_bytes += 4;
                } else {
                    out.write_all(&[fr[2], fr[1], fr[0]])
                        .map_err(|e| e.to_string())?;
                    data_bytes += 3;
                }
            }
    ...
    file.write_all(&((len - 8 + data_bytes + (data_bytes & 1)) as u32).to_le_bytes())
        .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn optimized_kernel_is_bit_identical_to_the_reference_implementation() {
        ...
        let x: Vec<f64> = (0..n)
            .map(|i| {
                ...
                rng ^= rng >> 7;
                rng ^= rng << 17;
                0.4 * (2.0 * std::f64::consts::PI * 1000.0 * i as f64 / 2_822_400.0).sin()
                    + 0.01 * ((rng >> 11) as f64 / (1u64 << 53) as f64 - 0.5)
            })
            .collect();
    ...
    fn dop_wav_preserves_the_dsd_bitstream_exactly() {
        ...
        for ch in 0..2 {
            assert_eq!(
                a24.samples_per_channel[ch].len(),
                a32.samples_per_channel[ch].len()
            );
            assert!(
                a24.samples_per_channel[ch]
                    .iter()
                    .zip(&a32.samples_per_channel[ch])
                    .all(|(x, y)| *y == *x << 8),
                "ch{ch}: 32bit=24bit<<8"
            );
        }
        ...
        for ch in 0..2 {
            ...
            let expected: Vec<u8> = dsf_bytes[92 + ch * DSF_BLOCK_BYTES..]
                .chunks(DSF_BLOCK_BYTES * 2)
                .flat_map(|c| c[..DSF_BLOCK_BYTES.min(c.len())].iter().copied())
                .take(valid)
                .map(|b| b.reverse_bits())
                .collect();
        ...
        assert_eq!(
            open_mqa::wav::decode_wav(&std::fs::read(&dop).unwrap())
                .unwrap()
                .sample_rate,
            176_400,
            "DSD64のDoPは176.4kHz"
    ...
    fn roundtrip_snr_db(multiplier: u32) -> f64 {
        ...
        assert!(
            (amplitude - INPUT_SCALE * 0.875).abs() < 0.03,
            "振幅0.875(sine既定0.125×volume=7)の0.5倍=約0.4375のはず(実際: {amplitude})"
        );
    ...
    fn real_dsd64_roundtrip_reproduces_the_sine_with_good_snr() {
        ...
        assert!(
            snr > 90.0,
            "DSD64でSNR 90dB超のはず(実測99.6dB、実際: {snr} dB)"
        );
    }
    ...
    fn real_all_dsd_rates_produce_correctly_sized_files_and_report_speed() {
        ...
        for mult in [64u32, 128, 256, 512, 1024] {
            ...
            convert_to_dsf(src.to_str().unwrap(), dsf.to_str().unwrap(), mult, None)
                .expect("convert");
            ...
            eprintln!(
                "DSD{mult}: 2秒の音声を {elapsed:.2}秒で変換(実時間の{:.1}倍)、{:.1} MB",
                elapsed / 2.0,
                size as f64 / 1e6
            );
            assert_eq!(
                size, expected,
                "DSD{mult}のファイルサイズが計算式どおりのはず"
            );
    ...
    fn real_dsd128_roundtrip_reproduces_the_sine_with_good_snr() {
        ...
        assert!(
            snr > 120.0,
            "DSD128でSNR 120dB超のはず(実測132.4dB、実際: {snr} dB)"
        );
    }
    ...
    fn real_music_to_dsd256_and_dop() {
        ...
        let dir = std::path::PathBuf::from(
            std::env::var("MAKE_DISK_DSD_OUT").expect("set MAKE_DISK_DSD_OUT"),
        );
        ...
        eprintln!(
            ...
            std::fs::metadata(&dsf).unwrap().len() as f64 / 1e6
        );
        let dop = dir.join("track.dop.wav");
        dsf_to_dop_wav(dsf.to_str().unwrap(), dop.to_str().unwrap(), 24).unwrap();
        eprintln!(
            "DoP WAV: {:.1} MB",
            std::fs::metadata(&dop).unwrap().len() as f64 / 1e6
        );
    }
}
...
#[cfg(test)]
pub(crate) fn write_exact_sine_wav(
    path: &std::path::Path,
    freq: f64,
    amplitude: f64,
    seconds: f64,
