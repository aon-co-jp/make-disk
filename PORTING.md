...
- yuv_to_rgb本体(Y/U/V読み込み+複数の即値係数を使った複数チャンネル
  計算+クランプ)を、この即値対応済みチェーンデコーダの上に実装する。
  ただし現状のチェーンデコーダは「全バッファが同じスレッドID添字」
  前提のままなので、クロマサブサンプリング(U/VがY解像度の半分)を
  伴うインデックス計算には、さらなるデコーダ拡張(添字に対する
  シフト/除算命令の認識)が必要になる見込み。
  代替案: まず4:4:4(サブサンプリング無し)版のyuv_to_rgbから着手し、
  実GPU検証してから、クロマサブサンプリング対応を次段階とする
  (このセッションと同じ「小さく検証してから広げる」方針を継続)。
