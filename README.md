...
- 有名な音声フォーマット(MP3等)・動画フォーマット(MP4等)への変換、
  ...
  複数選択可能。
- ビットレートは固定値、ディスク容量(CD/DVD/DVD DL/BD/BD DL/BDXL 4層
  128GB)から自動算出した最大値、または「最高音質・最高画質で記録する」
  モード(音声/動画フォーマット未選択でも実行可能——未選択時は自動で
  ロスレスWAVに変換しISO化、ディスクに収まる時間・データサイズも
  事前に算出して表示)から選択可能。
- 複数フォーマット・複数ファイルの変換は非同期・並列実行(1件ずつ
  待たずに複数のffmpegプロセスを同時起動)。
- 起動時に自動でGitHub Releasesの最新版を確認し、あれば日英併記の
  ダイアログでアップデートを選択・実行できる(デスクトップのみ)。
...
PATH上のものにフォールバックする。姉妹Rustリスペクト版
[rs-FFmpeg](https://github.com/aon-co-jp/rs-FFmpeg)・
[rs-xorriso](https://github.com/aon-co-jp/rs-xorriso)も実験的に同梱
しているが、アプリ本体はまだこれらを自動選択・利用しない(早期WIPの
ため、下記の制限を参照)。**本家xorriso・macOS版のffmpeg/ffprobeは
...
2026-09-16時点でv0.1.7まで公開済み。デスクトップ版はUIとRustコマンドの
...
Windows/Linux版インストーラーにはffmpeg/ffprobe・rs-FFmpeg/rs-xorriso
(実験的)を同梱済み(実際にインストーラーをビルド・サイレント
インストールして実機検証済み)。起動時の自動アップデート確認機能を
実装し、署名付きアップデートアーティファクト(`.sig`)の生成も実機
確認済み(実際にv0.1.7で更新が届くかのエンドツーエンド確認は次回)。
macOS版ffmpeg・本家xorrisoはまだ未対応(理由・設計は`CLAUDE.md`参照)。
実機でのディスク書き込みテスト(CD/DVD/Blu-ray)は未実施。詳細は
